...
def Merge_Sort(rugbyAgeData):
    if len(rugbyAgeData) > 1:
        mid = len(rugbyAgeData) // 2
        left = rugbyAgeData[:mid]
        right = rugbyAgeData[mid:]
        ...
        while x < len(left) and y < len(right):
            if left[x] <= right[y]:
                rugbyAgeData[z] = left[x]
                x += 1
            else:
                rugbyAgeData[z] = right[y]
                y += 1
            z += 1
