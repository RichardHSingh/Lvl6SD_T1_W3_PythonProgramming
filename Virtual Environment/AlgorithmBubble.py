...
def Bubble_Sort(rugbyAgeData):   
    ...
    for i in range(0,len(rugbyAgeData)-1):  

        # compares current element to elemnt beside it and swaps them in needed
        for j in range(len(rugbyAgeData)-1):  
            if(rugbyAgeData[j] > rugbyAgeData[j+1]):  
                temp = rugbyAgeData[j]  
                rugbyAgeData[j] = rugbyAgeData[j+1]  
                rugbyAgeData[j+1] = temp  
